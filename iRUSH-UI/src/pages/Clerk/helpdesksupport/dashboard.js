...
const dashboard = () => {
  ...
  useEffect(() => {
    const helpdeskAssignedTicketData = async () => {
      setLoading(true);
      ...
      await instance
        .get(
          `/tickets/helpdesk/assignedtickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${formattedDateFrom}&dateTo=${formattedDateTo}`
        )
        .then((response) => {
