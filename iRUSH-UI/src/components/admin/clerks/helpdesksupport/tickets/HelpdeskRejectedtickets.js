...
const HelpdeskRejectedtickets = () => {
  ...
  useEffect(() => {
    const fetchTicket = async () => {
      ...
      await instance
        .get(
          `/tickets/helpdesk/assignedtickets/rejectedtickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${formattedDateFrom}&dateTo=${formattedDateTo}`
        )
        .then((response) => {
          setTickets(response.data.assignedRejectedTicket);
          setRejectedTickets(response.data.filteredTickets);
