...
const HelpdeskOpentickets = () => {
  ...
  useEffect(() => {
    const fetchTicket = async () => {
      ...
      await instance
        .get(
          `/tickets/helpdesk/assignedtickets/opentickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${formattedDateFrom}&dateTo=${formattedDateTo}`
        )
        .then((response) => {
          setTickets(response.data.assignedOpenTicket);
          setOpentickets(response.data.filteredTickets);
        ...
        <div className={TICKETSTATSSTYLE["dashboard-container__wrapper"]}>
          <div className={TICKETSTATSSTYLE["content-ticketreport"]}>
            ...
            <div className={TICKETSTATSSTYLE["content-ticketreport__wrapper"]}>
              ...
              >
                <TableTicketData
                  header={"OPEN TICKET REPORT"}
                  dataTickets={opentickets}
                  ticketLoading={ticketDataLoading}
                  setDataTickets={setOpentickets}
            ...
            <div className={TICKETSTATSSTYLE["ticketreport-graphs"]}>
              ...
              >
                <DoughnutChart2
                  header={"OPEN TICKETS BASED ON UNIT"}
                  data={tickets}
                />
                <PieChart
                  header={"OPEN TICKETS BASED ON CATEGORY"}
                  data={tickets}
                />
              </div>
            </div>
...
export default HelpdeskOpentickets;
