...
const ItsupportOverduetickets = () => {
  ...
  useEffect(() => {
    const fetchTicket = async () => {
      ...
      await instance
        .get(
          `/tickets/itsupport/assignedtickets/overduetickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${formattedDateFrom}&dateTo=${formattedDateTo}`
        )
        .then((response) => {
          setTickets(response.data.assignedOverdueTicket);
          setOverdueTickets(response.data.filteredTickets);
        ...
        <div className={TICKETSTATSSTYLE["dashboard-container__wrapper"]}>
          <div className={TICKETSTATSSTYLE["content-ticketreport"]}>
            ...
            <div className={TICKETSTATSSTYLE["content-ticketreport__wrapper"]}>
              ...
              >
                <TableTicketData
                  header={"OVERDUE TICKET REPORT"}
                  dataTickets={overdueTickets}
                  ticketLoading={ticketDataLoading}
                  setDataTickets={setOverdueTickets}
            ...
            <div className={TICKETSTATSSTYLE["ticketreport-graphs"]}>
              ...
              >
                <DoughnutChart2
                  header={"OVERDUE TICKETS BASED ON UNIT"}
                  data={tickets}
                />
                <PieChart
                  header={"OVERDUE TICKETS BASED ON CATEGORY"}
                  data={tickets}
                />
              </div>
            </div>
...
export default ItsupportOverduetickets;
