...
const ItsupportReopenedtickets = () => {
  ...
  useEffect(() => {
    const fetchTicket = async () => {
      ...
      await instance
        .get(
          `/tickets/itsupport/assignedtickets/reopenedtickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${formattedDateFrom}&dateTo=${formattedDateTo}`
        )
        .then((response) => {
          setTickets(response.data.assignedReopenedTickets);
          setReopenedtickets(response.data.filteredTickets);
        ...
        <div className={TICKETSTATSSTYLE["dashboard-container__wrapper"]}>
          <div className={TICKETSTATSSTYLE["content-ticketreport"]}>
            ...
            <div className={TICKETSTATSSTYLE["content-ticketreport__wrapper"]}>
              ...
              >
                <TableTicketData
                  header={"REOPENED TICKET REPORT"}
                  dataTickets={reopenedTickets}
                  ticketLoading={ticketDataLoading}
                  setDataTickets={setReopenedtickets}
            ...
            <div className={TICKETSTATSSTYLE["ticketreport-graphs"]}>
              ...
              >
                <DoughnutChart2
                  header={"REOPENED TICKETS BASED ON UNIT"}
                  data={tickets}
                />
                <PieChart
                  header={"REOPENED TICKETS BASED ON CATEGORY"}
                  data={tickets}
                />
              </div>
            </div>
...
export default ItsupportReopenedtickets;
