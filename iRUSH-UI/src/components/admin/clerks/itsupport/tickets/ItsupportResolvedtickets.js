...
const ItsupportResolvedtickets = () => {
  ...
  useEffect(() => {
    const fetchTicket = async () => {
      ...
      await instance
        .get(
          `/tickets/itsupport/assignedtickets/resolvedtickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${formattedDateFrom}&dateTo=${formattedDateTo}`
        )
        .then((response) => {
          setTickets(response.data.assignedResolvedTicket);
          setResolvedtickets(response.data.filteredTickets);
        ...
        <div className={TICKETSTATSSTYLE["dashboard-container__wrapper"]}>
          <div className={TICKETSTATSSTYLE["content-ticketreport"]}>
            ...
            <div className={TICKETSTATSSTYLE["content-ticketreport__wrapper"]}>
              ...
              >
                <TableTicketData
                  header={"RESOLVED TICKET REPORT"}
                  dataTickets={resolvedTickets}
                  ticketLoading={ticketDataLoading}
                  setDataTickets={setResolvedtickets}
            ...
            <div className={TICKETSTATSSTYLE["ticketreport-graphs"]}>
              ...
              >
                <DoughnutChart2
                  header={"RESOLVED TICKETS BASED ON UNIT"}
                  data={tickets}
                />
                <PieChart
                  header={"RESOLVED TICKETS BASED ON CATEGORY"}
                  data={tickets}
                />
              </div>
            </div>
...
export default ItsupportResolvedtickets;
