...
const ItsupportVoidedtickets = () => {
  ...
  useEffect(() => {
    const fetchTicket = async () => {
      ...
      await instance
        .get(
          `/tickets/itsupport/assignedtickets/voidedtickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${formattedDateFrom}&dateTo=${formattedDateTo}`
        )
        .then((response) => {
          setTickets(response.data.assignedVoidedTicket);
          setVoidedTickets(response.data.filteredTickets);
        ...
        <div className={TICKETSTATSSTYLE["dashboard-container__wrapper"]}>
          <div className={TICKETSTATSSTYLE["content-ticketreport"]}>
            ...
            <div className={TICKETSTATSSTYLE["content-ticketreport__wrapper"]}>
              ...
              >
                <TableTicketData
                  header={"VOIDED TICKET REPORT"}
                  dataTickets={voidedTickets}
                  ticketLoading={ticketDataLoading}
                  setDataTickets={setVoidedTickets}
            ...
            <div className={TICKETSTATSSTYLE["ticketreport-graphs"]}>
              ...
              >
                <DoughnutChart2
                  header={"REJECTED TICKETS BASED ON UNIT"}
                  data={tickets}
                />
                <PieChart
...
export default ItsupportVoidedtickets;
