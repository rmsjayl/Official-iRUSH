...
const Voidedtickets = () => {
  ...
  useEffect(() => {
    const getOpentickets = async () => {
      ...
      await instance
        .get(
          `/tickets/voidedtickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${formattedDateFrom}&dateTo=${formattedDateTo}`
        )
        .then((res) => {
          setTickets(res.data.voidedTickets);
          setLimit(res.data.limit);
          setTotal(res.data.total);
          setVoidedtickets(res.data.filteredTickets);
        })
        .catch((error) => {
        ...
        <div className={TICKETSTATSSTYLE["dashboard-container__wrapper"]}>
          <div className={TICKETSTATSSTYLE["content-ticketreport"]}>
            ...
            <div className={TICKETSTATSSTYLE["ticketreport-graphs"]}>
              ...
              >
                <DoughnutChart2
                  header={"VOIDED TICKETS BASED ON UNITS"}
                  data={tickets}
                />
                <PieChart
                  header={"VOIDED TICKETS BASED ON CATEGORY"}
                  data={tickets}
                />
              </div>
            </div>
