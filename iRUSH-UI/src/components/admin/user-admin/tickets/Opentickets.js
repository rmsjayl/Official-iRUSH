...
const Opentickets = () => {
  ...
  useEffect(() => {
    const getOpentickets = async () => {
      ...
      await instance
        .get(
          `/tickets/opentickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${formattedDateFrom}&dateTo=${formattedDateTo}`
        )
        .then((res) => {
          setTickets(res.data.openTickets);
          setLimit(res.data.limit);
          setTotal(res.data.total);
          setOpentickets(res.data.filteredTickets);

          console.log(res.data.filteredTickets);
        })
        .catch((error) => {
        ...
        <div className={TICKETSTATSSTYLE["dashboard-container__wrapper"]}>
          <div className={TICKETSTATSSTYLE["content-ticketreport"]}>
            ...
            <div className={TICKETSTATSSTYLE["ticketreport-graphs"]}>
              ...
              >
                <DoughnutChart2
                  header={"OPEN TICKETS CREATED BASED ON UNIT"}
                  data={tickets}
                />
                <PieChart
                  header={"OPEN CREATED BASED ON CATEGORY"}
                  data={tickets}
                />
              </div>
            </div>
