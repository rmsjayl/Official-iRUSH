...
const Overduetickets = () => {
  ...
  useEffect(() => {
    const getOverdueTickets = async () => {
      ...
      await instance
        .get(
          `/tickets/overduetickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${formattedDateFrom}&dateTo=${formattedDateTo}`
        )
        .then((res) => {
          setTickets(res.data.overdueTickets);
          setLimit(res.data.limit);
          setTotal(res.data.total);
          setOverduetickets(res.data.filteredTickets);
        })
        .catch((error) => {
        ...
        <div className={TICKETSTATSSTYLE["dashboard-container__wrapper"]}>
          <div className={TICKETSTATSSTYLE["content-ticketreport"]}>
            ...
            <div className={TICKETSTATSSTYLE["ticketreport-graphs"]}>
              ...
              >
                <DoughnutChart2
                  header={"OVERDUE TICKETS BASED ON UNITS"}
                  data={tickets}
                />
                <PieChart
