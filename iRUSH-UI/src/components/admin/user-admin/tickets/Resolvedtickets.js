...
const Resolvedtickets = () => {
  ...
  useEffect(() => {
    const getResolvedTickets = async () => {
      ...
      await instance
        .get(
          `/tickets/resolvedtickets?priority=${priority.priority}&page=${page}&limit=${limit}&search=${search}&ticketCategory=${category.category}&dateFrom=${formattedDateFrom}&dateTo=${formattedDateTo}`
        )
        .then((res) => {
          setTickets(res.data.resolvedTickets);
          setLimit(res.data.limit);
          setTotal(res.data.total);
          setResolvedtickets(res.data.filteredTickets);
        })
        .catch((error) => {
        ...
        <div className={TICKETSTATSSTYLE["dashboard-container__wrapper"]}>
          <div className={TICKETSTATSSTYLE["content-ticketreport"]}>
            ...
            <div className={TICKETSTATSSTYLE["ticketreport-graphs"]}>
              ...
              >
                <DoughnutChart2
                  header={"RESOLVED TICKETS BASED ON UNITS"}
                  data={tickets}
                />
                <PieChart
