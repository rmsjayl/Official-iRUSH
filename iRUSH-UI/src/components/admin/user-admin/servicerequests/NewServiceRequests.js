...
const NewServiceRequests = () => {
  ...
  const [newServiceRequests, setNewServiceRequests] = useState([]);
  ...
  useEffect(() => {
    const getServiceRequests = async () => {
      ...
      await instance
        .get(
          `/tickets/servicerequests?page=${page}&limit=${limit}&search=${search}&serviceCategory=${category.category}&dateFrom=${formattedDateFrom}&dateTo=${formattedDateTo}`
        )
        .then((res) => {
          setServiceRequests(res.data.service);
          setLimit(res.data.limit);
          setTotal(res.data.total);
          setNewServiceRequests(res.data.filteredServiceRequests);
        })
        .catch((error) => {
        ...
        <div className={SERVICEREQUESTSSTYLE["dashboard-container__wrapper"]}>
          <div className={SERVICEREQUESTSSTYLE["content-servicerequestreport"]}>
            ...
            >
              ...
              >
                <TableServiceRequestData
                  header={"NEW SERVICE REQUESTS"}
                  newServiceRequest
                  serviceRequestsLoading={serviceRequestsLoading}
                  dataServiceRequests={newServiceRequests}
                  setDataServiceRequests={setNewServiceRequests}
            ...
            >
              ...
              >
                <DoughnutChart2
                  header={"NEW SERVICE REQUESTS BASED ON UNIT"}
                  data={serviceRequests}
                />
                <PieChart3
                  header={"NEW SERVICE REQUESTS CREATED BASED ON CATEGORY"}
                  data={serviceRequests}
                />
              </div>
            </div>
